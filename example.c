...
int main(int argc, char **argv) {
  ...
  cyn_voice wow = cyn_new_voice("wow", &osc1, pattern1, &lfo1, NULL);
  cyn_add_voice(wow);
  ...
  cyn_voice v2 = cyn_new_voice("v2", &osc2, pattern2, NULL, &env2);
  cyn_add_voice(v2);
  ...
  while (CYNTHER_RUNNING) {
    for (int i = 0; i < MAX_VOICES; i++) {
      float new_sustain = 0.4f + 0.4f * sinf(cyn_time() + (M_PI * 3.0f / 2.0f));
